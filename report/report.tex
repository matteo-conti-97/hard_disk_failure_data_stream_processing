...
\subsubsection{Query 1}
Si può notare che per la prima query i risultati per tutte le finestre sono abbastanza in linea tra loro per quanto rigaurda il throughput. Invece per quanto riguarda la latenza essa va a comparire sempre più tardi mano a mano che la finestra aumenta, questo probabilmente è dovuto al fatto che nelle finestre più piccole ci sono più parti in cui il sistema deve mandare l'output sul topic di uscita e quindi rallenta l'inizio dell'elaborazione successiva portando ad un aumento della latenza. Inoltre si può notare che l'andamento del throughput è essenzialmente costante.
\begin{figure}[H]
    \centering
